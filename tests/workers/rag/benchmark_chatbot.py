...
QUESTIONS = [
    "D2 비자로 전환하는 방법을 알려줘",
    "유학생 장학금 관련한 공지를 정리해줘",
    "외국인 등록증은 어떻게 발급 받을 수 있어 ?",
...
def _check_env() -> None:
    ...
    try:
        ...
        _skip_or_exit(f"벤치마크 실행 환경이 준비되지 않았습니다: {exc}")
...
def test_benchmark_chatbot_basic():
    ...
    async def _run():
        ...
        for q in QUESTIONS:
            ...
            metrics = getattr(service, "last_metrics", {}) or {}
        ...
        for item in per_question:
            ...
            rows_html.append(
                ...
                f"<td>{docs_count}</td>"
                f"<td>{_format_number(top_score)}</td>"
                f"<td>{_format_number(avg_score)}</td>"
                f"<td>{context_tokens_est}</td>"
                f"<td><pre>{_format_usage(usage)}</pre></td>"
