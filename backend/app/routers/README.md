...
| 클래스 | 필드 | 설명 |
|--------|------|------|
| `NoticeCreateRequest` | `notice_id?`, `keyword_id?`, `title`, `body`, `preview?`, `source?`, `url?`, `hash?`, `is_processed?`, `deadline?`, `published_at?` | 공지 생성 요청 본문 |
...
| `GET` | `/notices/{notice_id}` | `notice_id: UUID` | `{id, title, body, preview, url, keyword_id, keyword, published_at}` | 공지 상세 조회 |
