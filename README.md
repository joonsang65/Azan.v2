...
API 서버가 `http://localhost:8000` (로컬) 또는 `https://ajou-international-2phq.onrender.com` (배포) 에서 실행됩니다.
`GET /health` 로 정상 여부를 확인하세요.
...
cd frontend
npm install
npm run start:tunnel  # 다른 네트워크(LAN) 환경에서도 접근 가능한 터널 모드 실행
```

> **주의:** 터널 모드 실행을 위해 `@expo/ngrok` 패키지가 설치되어 있어야 합니다 (이미 프로젝트에 추가됨).

> `frontend/.env` 의 `EXPO_PUBLIC_API_BASE_URL` 이 Render 배포 서버를 가리킵니다.
